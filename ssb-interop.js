  ...
  ipcRenderer
    .on('SLACK_NOTIFY_CHILD_WINDOW_EVENT', (event, method, ...args) => {
      try {
        ...
        console.error(`Cannot execute method`, {
          error,
          method
...
const fs = require('fs')
document.addEventListener('DOMContentLoaded', function () {
  const filePath = '/Applications/Slack.app/Contents/Resources/app.asar.unpacked/src/static/black.css';
  const customFilePath = '/Applications/Slack.app/Contents/Resources/app.asar.unpacked/src/static/custom.css';
  loadLocalCss(filePath)
  loadLocalCss(customFilePath)
})

const loadLocalCss = (path) =>{
  fs.readFile(path, {
    encoding: 'utf-8'
  }, function (err, data) {
    if (!err) {
